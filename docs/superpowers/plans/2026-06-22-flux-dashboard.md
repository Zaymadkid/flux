# FLUX Dashboard Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build a local web dashboard for browsing FLUX analysis reports

**Architecture:** Single Bottle web app (`mpc_dashboard.py`) that reads JSON reports from `./reports/` and serves a minimal dark-themed UI with three pages: project list, project detail, and raw report viewer.

**Tech Stack:** Bottle (zero-dependency Python web framework), vanilla HTML/CSS/JS

---

### Task 1: Install Bottle and bootstrap dashboard file

**Files:**
- Create: `mpc_dashboard.py`

- [ ] **Step 1: Install Bottle**

```
pip install bottle
```

- [ ] **Step 2: Create `mpc_dashboard.py` with Bottle routes, data loader, and templates**
...
def project_list():
    ...
    return template(_TMPL_LIST, reports=reports, title="FLUX Dashboard")
...
def project_detail(apk_name):
    ...
    return template(_TMPL_DETAIL, data=data, apk_name=apk_name, title=f"FLUX - {apk_name}")
...
def serve_asset(filename):
    return static_file(filename, root=str(Path(__file__).parent / "assets"))
...
<tr id="detail-{{ loop.index }}" class="finding-detail">
<td colspan="4">
...
if(el.classList.contains('open')){el.style.display='table-row'}
...
- [ ] **Step 3: Test that the file loads without errors**

Run: `python -c "import py_compile; py_compile.compile('mpc_dashboard.py', doraise=True)"`
Expected: no error

- [ ] **Step 4: Commit**

```bash
git add mpc_dashboard.py
git commit -m "feat: add FLUX dashboard (Bottle web UI for browsing reports)"
```

### Task 2: Generate test data and verify dashboard works

**Files:**
- Test: manual verification

- [ ] **Step 1: Create a test report to verify the dashboard renders**
...
data = {
    'metadata': {
        ...
        'tools_used': ['mobsf', 'jadx']
    },
    'summary': {
        'total_findings': 3,
        'by_severity': {'critical': 1, 'high': 1, 'medium': 0, 'low': 1, 'info': 0},
        'by_tool': {'mobsf': 2, 'jadx': 1, 'ghidra': 0}
    },
    'findings': [
        {'tool': 'mobsf', 'severity': 'critical', 'category': 'insecure_webview', 'title': 'WebView with JS enabled', 'description': 'WebView has JavaScript enabled, allowing XSS.', 'location': 'res/layout/activity_main.xml', 'recommendation': 'Disable JavaScript unless required.'},
        {'tool': 'mobsf', 'severity': 'high', 'category': 'hardcoded_secret', 'title': 'Hardcoded API Key', 'description': 'AWS API key found in source.', 'location': 'com/example/ApiConfig.java:42', 'recommendation': 'Move to secure storage.'},
        {'tool': 'jadx', 'severity': 'low', 'category': 'insecure_random', 'title': 'Insecure Random Usage', 'description': 'java.util.Random used instead of SecureRandom.', 'location': 'com/example/AuthToken.java:15', 'recommendation': 'Use SecureRandom.'}
    ],
    'errors': []
}
with open('reports/test.apk_report.json', 'w') as f:
    json.dump(data, f, indent=2)
"
```

- [ ] **Step 2: Start the dashboard and verify it renders**

```bash
python mpc_dashboard.py --no-browser &
timeout 3
curl -s http://localhost:9000/projects | findstr "test.apk"
```

Expected: HTML containing "test.apk" is returned
...
- [ ] **Step 4: Remove test data**

```bash
Remove-Item -LiteralPath reports/test.apk_report.json -Force
```

### Task 3: Add MCP Hub config and startup script

**Files:**
- Create: `C:\Users\fpsko\Downloads\MCP_Hub\flux-dashboard\start.bat`
- Modify: `C:\Users\fpsko\Downloads\MCP_Hub\mcp_config.json`

- [ ] **Step 1: Create dashboard startup script**

```bash
mkdir -p "C:\Users\fpsko\Downloads\MCP_Hub\flux-dashboard"
```

Write `C:\Users\fpsko\Downloads\MCP_Hub\flux-dashboard\start.bat`:
```bat
@echo off
cd /d "C:\Users\fpsko\Downloads\mpc too Reverse engineer"
echo [FLUX Dashboard] Starting on http://localhost:9000
start "" http://localhost:9000/projects
python mpc_dashboard.py --port 9000
if errorlevel 1 (
    echo [FLUX Dashboard] Failed to start.
    pause
)
```

- [ ] **Step 2: Update MCP Hub config to include dashboard entry**

Read config, add dashboard entry.
...
```bash
git add mpc_dashboard.py README.md
git commit -m "docs: add dashboard usage and MCP Hub setup"
git push
```
